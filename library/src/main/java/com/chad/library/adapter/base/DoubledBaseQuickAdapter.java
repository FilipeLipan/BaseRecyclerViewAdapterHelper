...
import com.chad.library.adapter.base.loadmore.DoubleLoadMoreView;
import com.chad.library.adapter.base.loadmore.DoubleSimpleLoadMoreView;
 ...
 */
public abstract class DoubledBaseQuickAdapter<T, K extends DoubledBaseViewHolder> extends PagedListAdapter<T,K> {

    //load more
    private boolean mNextLoadEnable = false;
    private boolean mLoadMoreEnable = false;
    private boolean mLoading = false;
    private DoubleLoadMoreView mLoadMoreView = new DoubleSimpleLoadMoreView();
    ...
    protected static final String TAG = DoubledBaseQuickAdapter.class.getSimpleName();
...
//    protected List<T> mData;
     ...
     */
    public void setLoadMoreView(DoubleLoadMoreView loadingView) {
    ...
    public void loadMoreEnd(boolean gone) {
        ...
        if (gone) {
            ...
            mLoadMoreView.setLoadMoreStatus(DoubleLoadMoreView.STATUS_END);
    ...
    public void loadMoreFail() {
        ...
        mLoading = false;
        mLoadMoreView.setLoadMoreStatus(DoubleLoadMoreView.STATUS_FAIL);
        notifyItemChanged(getLoadMoreViewPosition());
    }
...
//        this.mData = data == null ? new ArrayList<T>() : data;
    ...
    public DoubledBaseQuickAdapter(@Nullable List<T> data, @NonNull DiffUtil.ItemCallback<T> diffCallback) {
        this(0, data, diffCallback);
    }

    public DoubledBaseQuickAdapter(@LayoutRes int layoutResId, @NonNull DiffUtil.ItemCallback<T> diffCallback) {
...
//    }
    ...
    private K getLoadingView(ViewGroup parent) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mLoadMoreView.getLoadMoreStatus() == DoubleLoadMoreView.STATUS_FAIL) {
                    notifyLoadMoreToLoading();
                }
                if (mEnableLoadMoreEndClick && mLoadMoreView.getLoadMoreStatus() == DoubleLoadMoreView.STATUS_END) {
                    notifyLoadMoreToLoading();
                }
            }
        });
    ...
    public void notifyLoadMoreToLoading() {
        if (mLoadMoreView.getLoadMoreStatus() == DoubleLoadMoreView.STATUS_LOADING) {
            return;
        }
        mLoadMoreView.setLoadMoreStatus(DoubleLoadMoreView.STATUS_DEFAULT);
        notifyItemChanged(getLoadMoreViewPosition());
    ...
    public void onBindViewHolder(K holder, int position) {
        ...
        //Do not move position, need to change before DoubleLoadMoreView binding
    ...
    private void bindViewClickListener(final DoubledBaseViewHolder baseViewHolder) {
    ...
    public void setOnItemClick(View v, int position) {
        getOnItemClickListener().onItemClick(DoubledBaseQuickAdapter.this, v, position);
    }
    ...
    public boolean setOnItemLongClick(View v, int position) {
        return getOnItemLongClickListener().onItemLongClick(DoubledBaseQuickAdapter.this, v, position);
    }
    ...
    /**
     * if you want to use subclass of DoubledBaseViewHolder in the adapter,
    ...
    protected K createBaseViewHolder(View view) {
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (K) new DoubledBaseViewHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (K) new DoubledBaseViewHolder(view);
    }
        ...
        if (type instanceof ParameterizedType) {
            ...
            for (Type temp : types) {
                if (temp instanceof Class) {
                    ...
                    if (DoubledBaseViewHolder.class.isAssignableFrom(tempClass)) {
                        return tempClass;
                    }
                } else if (temp instanceof ParameterizedType) {
                    Type rawType = ((ParameterizedType) temp).getRawType();
                    if (rawType instanceof Class && DoubledBaseViewHolder.class.isAssignableFrom((Class<?>) rawType)) {
...
//    }
    ...
    private void autoLoadMore(int position) {
        ...
        if (mLoadMoreView.getLoadMoreStatus() != DoubleLoadMoreView.STATUS_DEFAULT) {
            return;
        }
        mLoadMoreView.setLoadMoreStatus(DoubleLoadMoreView.STATUS_LOADING);
        if (!mLoading) {
            mLoading = true;
            if (getRecyclerView() != null) {
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, @IdRes int viewId) {
        ...
        DoubledBaseViewHolder viewHolder = (DoubledBaseViewHolder) recyclerView.findViewHolderForLayoutPosition(position);
        if (viewHolder == null) {
            return null;
        }
